...
export class ClientBlacklistPage implements OnInit {
  ...
  async loadBlacklistData() {
    this.isLoading = true
    let params = {page: this.currentPage, limit: this.functionMain.limitHistory, is_visitor: this.isVisitor, name: this.filter.name, contact_search: this.filter.contact, vehicle_number: this.filter.vehicle_number}
    this.visitorData = []
    this.vehicleData = []
    this.blacklistData = []
  ...
  callResident(data: any){
    console.log(data)
    // if (this.project_config.is_industrial) {
      this.webRtcService.createOffer(false, data.ban_requestor_id[0], false, false);
    // } else {
