...
export class PaymentFormVehiclePage implements OnInit {
      ...
      (response: any) => {
        ...
        if (result) {
          this.vehicleData = {
            ...
            vehicleType: result.vehicle_type_display,
            vehicleMake: result.vehicle_make_display,
  ...
  onUploadPaymentReceipt(event: any) {
    ...
    if (data) {
      this.selectedPaymentReceiptFileName = this.truncateFileName(data.name, 30); // Store the selected file name
