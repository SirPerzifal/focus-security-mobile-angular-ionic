...
@Component({
  selector: 'app-contractor-inviting-from-history',
  templateUrl: './contractor-inviting-from-history.page.html',
  styleUrls: ['./contractor-inviting-from-history.page.scss'],
})
export class ContractorInvitingFromHistoryPage implements OnInit {
  historyData: any[] = [];
      ...
      if (existingInvitees) {
        this.selectedInvitees = existingInvitees.filter((existing: any) => 
          this.historyData.some(history => 
            history.contractor_name === existing.contractor_name && 
  ...
  loadDistinctInviteHistory() {
    this.mainApi.endpointMainProcess({}, 'get/contractor_history').subscribe({
      next: (response: any) => {
      ...
      error: (error) => {
        this.functionMain.presentToast('Error loading history data', 'danger');
  ...
  isInviteeExisted(invitee: any): boolean {
    return this.existingInvitees.some(existingInvitee => 
      existingInvitee.contractor_name === invitee.contractor_name &&
  ...
  toggleSelect(invitee: any) {
    ...
    const index = this.selectedInvitees.findIndex(
      selected => 
        selected.contractor_name === invitee.contractor_name && 
    ...
    const ifExist = this.existingInvitees.findIndex(
      existing => 
        existing.contractor_name === invitee.contractor_name && 
  ...
  confirmSelection() {
    ...
    this.router.navigate(['/contractor-inviting-form'], navigationExtras);
  }

  backToPrevPage() {
    this.router.navigate(['contractor-inviting-form'])
  }
